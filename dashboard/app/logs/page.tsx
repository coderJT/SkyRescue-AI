...
import { type LogType } from '@/lib/mock-data'
...
function AgentLogsContent() {
  ...
  const missionHistory = useMemo(() => [{ id: missionId }], [missionId])

  // Filter entries based on search and active filters
  const filteredEntries = missionLogs.filter(entry => {
    // Search filter
    if (searchQuery) {
