...
import { postMissionAction, useDashboardState } from '@/lib/backend-state'
...
function MissionControlContent() {
  ...
  const { state, missionStatus, uiDrones, refresh } = useDashboardState()
  const [speedMultiplier, setSpeedMultiplier] = useState(1)
  const [copied, setCopied] = useState(false)

  const summary = state.summary
  const missionId = useMemo(() => {
    const raw = state.world?.mission_id
    return raw ? String(raw) : 'MSN-LIVE'
  }, [state.world])
  const elapsedTime = Math.round(summary?.elapsed_seconds ?? 0)
  const mapDrones = uiDrones.length > 0 ? uiDrones : []
  ...
  const handleStart = async () => {
    await postMissionAction('start')
    await refresh()
  }
  const handlePause = async () => {
    await postMissionAction('pause', { paused: true })
    await refresh()
  }
  const handleReset = async () => {
    await postMissionAction('reset')
    await refresh()
  }
  ...
  return (
    ...
    >
          ...
          <div className="flex items-center gap-3">
            <div className="hidden items-center gap-2 rounded-full border border-border bg-card/80 px-3 py-1.5 sm:flex">
              <span className="relative flex h-2 w-2">
                <span
                  className={cn(
                    ...
                    state.connected ? 'bg-success' : 'bg-error'
                  )}
                />
                <span
                  className={cn(
              ...
              <span className="font-mono text-xs text-muted-foreground">
                {state.connected ? 'MCP Connected' : 'MCP Disconnected'}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="relative h-full w-full dot-pattern bg-muted/30">
          ...
          {mapDrones.map((drone) => (
            <div
              ...
              style={{
                left: `${Math.max(8, Math.min(88, 20 + (drone.coordinates.x / 200) * 60))}%`,
                top: `${Math.max(8, Math.min(88, 20 + (drone.coordinates.z / 200) * 60))}%`,
              }}
      ...
      <div className="absolute bottom-6 left-6 z-40">
        <div className="flex gap-3">
          ...
          <div className="rounded-xl border border-border bg-card/95 px-4 py-3 shadow-lg backdrop-blur-sm">
            ...
            <p className="text-2xl font-semibold text-primary">{summary?.coverage_pct ?? 0}%</p>
          </div>
          ...
          <div className="rounded-xl border border-border bg-card/95 px-4 py-3 shadow-lg backdrop-blur-sm">
            ...
            <p className="text-2xl font-semibold text-success">
              {summary?.found_survivors ?? 0}/{summary?.total_survivors ?? 0}
            </p>
          </div>
          ...
          <div className="hidden rounded-xl border border-border bg-card/95 px-4 py-3 shadow-lg backdrop-blur-sm sm:block">
            ...
            <p className="text-2xl font-semibold text-foreground">
              {summary?.active_drones ?? 0}/{summary?.drone_count ?? 0}
            </p>
          </div>
        </div>
      </div>
      ...
      {state.error && (
        <div className="absolute top-16 left-1/2 z-40 -translate-x-1/2 rounded-lg border border-error/30 bg-error/10 px-4 py-2 text-sm text-error">
          {state.error}
        </div>
      )}
    </div>
